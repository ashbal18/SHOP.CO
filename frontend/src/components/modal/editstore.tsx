...
export default function EditStoreModal({ open, onClose, onUpdate, token, storeToEdit }: EditStoreModalProps) {
  // Hooks harus dideklarasikan selalu, tidak boleh di bawah kondisi
  ...
  useEffect(() => {
    if (storeToEdit) {
      ...
      setAdminId(storeToEdit.adminId);
      setCityId(Number(storeToEdit.city_id));
      setLatitude(storeToEdit.latitude);
      setLongitude(storeToEdit.longitude);
    }
  }, [storeToEdit]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error("Gagal memperbarui toko:", err);
    } finally {
      setLoading(false);
    }
  };

  // Jika modal tidak terbuka atau store tidak ada, langsung return
  if (!open || !storeToEdit) return null;

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div className="bg-white rounded-lg p-6 w-full max-w-2xl shadow-lg">
        <h2 className="text-xl font-semibold mb-4">Edit Toko</h2>
